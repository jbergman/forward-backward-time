...
\begin{equation}
\Pr(\y \given \bs{\rho}) = \fv{t}\bv{t}' =\sum_i \Pr(\x{t}=i \given \bs{\rho}) \Pr(\y \given \x{t}=i) = \langle \fv{t}, \bv{t} \rangle_{\bs{\rho}},
\end{equation}
where $\langle , \rangle>$ denotes an inner product.
% TODO: This may be refined because the inner product depends on \rho (subscript).
It follows that forward and backward transition matrices, \ie\ $\mathbf{T}$ and its transpose $\mathbf{T}'$, are adjoint since
