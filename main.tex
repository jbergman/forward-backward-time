\documentclass[preprint]{elsarticle}fadsfadsf
...
The Wright-Fisher and Moran models are Markov processes, where discrete population allele frequencies are iterated forward in time conditional on population genetic forces such as mutation, selection, and drift. Often large population sizes are assumed, such that forward iteration can be replaced by the continuous Kolmogorov forward diffusion. Data are typically aligned sequences of a sample of individuals from one or more populations from the present. The task is to infer population genetic parameters or the population history from these data. In highly recombining regions, sites can be assumed to evolve independently, such that the entire information is contained in the site frequency spectrum (SFS) of a single population or the joint SFS of many populations. For bi-allelic sites, hypergeometric or binomial likelihoods are usually assumed. With forward processes, \ie\ the Wright-Fisher, the Moran, or the forward diffusion model, population allele frequency distributions in and out of equilibrium may be obtained. These can be combined with the likelihoods of SFS to obtain joint distributions of sample and population allele frequencies. By integrating out population allele frequencies, marginal likelihoods may be obtained from the SFS of a single population to infer population genetic parameters (\eg\ the scaled mutation rate, mutation bias, or scaled selection strength). %CV: We need the backward processes here, otherwise the following sentence makes no sense.
The backward process consists of, in the discrete case, iteration of the transpose of the forward transition matrix or, in the continuous case, of the backward diffusion equation. Combining the forward and backward processes, as in the forward-backward algorithm of hidden Markov models, the probability distribution of population allele frequencies can be inferred at all times into the past. Additionally, marginal likelihoods of samples from multiple populations that split in the past, \ie\ of joint SFS, may thus be obtained. In this article, we present the general theory, introduce reversible and irreversible models, and illustrate with applications.
\end{abstract}
\begin{keyword}
...
With data from two or more populations that split some time in the past, \ie\ with joint SFS, inference is more complicated. Models iterating forward in time, \eg\ the Wright-Fisher or Moran models or continuous diffusion models, have rarely been used for this task. An exception is the use of the Moran model in phylogenetic inference \citep{DeMaio2015}. In this case, summation over all $N+1$ ancestral states is necessary at each node, \ie\ at each binary split into two populations. Such summation becomes difficult with large $N$. Note that information is lost if $M>N$. Sample sizes $M$ can be large for sequence data from pooled individuals or from many populations. Such data are therefore difficult to analyze accurately with forward methods.

Backward processes allow pushing the conditioning on the population allele frequency backward in time, \ie\ they allow calculation of the likelihood $\Pr(Y\given X_t, \dots)$ with $t<0$. Backward processes consist of, in the discrete case, iteration of the transpose of the forward transition matrix; in the continuous case, of the backward diffusion equation. Combining the forward and backward processes in the forward-backward algorithm, the probability distribution of population allele frequencies conditional on the sample allele frequencies $\Pr(X_t\given Y,\dots)$ can be inferred at all times $t$ until present. This may be an aim in itself. Additionally, it is thus possible to calculate the marginal likelihoods of samples from multiple splitting populations, \ie\ from joint site frequency spectra.

In this article, we use backward processes to conveniently calculate probabilities of population allelic proportions backward in time conditional on SFS from the present. In discrete time, the algorithm is a variant of the forward-backward algorithm and thus makes use of the dynamic programming algorithm. The method can also be used to calculate likelihoods of joint SFS. Furthermore, we introduce biallelic population genetic models with mutations only from the boundaries that are variants of the infinite site or Poisson-random-fields models \citep{Kimu69,Sawy92}. The Markov chains of the models have stationary distributions but are not time-reversible. We also analyze the bi-allelic model with reversible Markov chain. For the discrete models, we iterate transition probabilities, for the continuous diffusion models, we use orthogonal polynomials. 
...
Assume a mutation model where the mutation rate is small, such that at most one mutation is segregating per site. The transition matrix $\mathbf{T}$ is defined as follows. Given a monomorphic site ($i=0$) and a time-homogenous mutation rate $\theta$, transition probabilities at the boundary are 
\begin{equation}
\begin{cases}
...
In this section, we derive the forward and backward diffusion equation from the forward and backward transition probabilities of the decoupled Moran model and show the connections between the discrete and continuous models.
...
With the  boundary mutation model, the scaled mutation rate $\theta=(\mu_0+\mu_1)N$ is assumed to be much less than one, such that away from the immediate vicinity of the boundaries, the forces of drift are much stronger than mutational forces. Only very close to the boundaries, in regions $0\leq x \leq\alpha\theta$ and $(1-\beta\theta) \leq x \leq 1$, mutation can overcome drift (see Fig.~1 in \citet{Vogl15}). 

\subsubsection{Reversible model: forward in time}

Forward in time, the mutation term in the diffusion equation can be replaced by either a Heaviside function, before differentiation with respect to space, or a delta function, after differentiation with respect to space. We chose the latter variant:
...
\begin{equation}
\begin{split}
    \pi(x)&=\left(b_0(\tau=\infty)-\alpha\beta\theta \lim_{N\to\infty}\int_{\tfrac1N}^{\tfrac{N-1}N} \frac1x\,dx\right)\delta(x)\\ &\qquad+\alpha\beta\theta\frac{1}{x(1-x)} +\\
    \left(b_1(\tau=\infty)-\lim_{N\to\infty}\alpha\beta\theta \int_{\tfrac1N}^{\tfrac{N-1}N} \frac1{1-x}\,dx\right)\delta(x-1)\,,
\end{split}
\end{equation}
...
Because the system is in detailed balance, the backward diffusion equation can be obtained from the forward diffusion equation by multiplication with the equilibrium distribution. The relationship between the boundaries forward and backward in time then are
\begin{equation}
\begin{cases}
\beta b_0^{*}(\tau)&= b_0(\tau)\\ 
\alpha b_1^{*}(\tau)&=b_1(\tau)\,.
...
\end{split}
\end{equation}
where we multiplied with $\alpha\beta\theta$. This also necessitates that the augmented eigenfunctions with $i=0$ and $i=1$, and thus $b_0^{*}(\tau)$ and $b_1^{*}(\tau)$, are multiplied with the same proportionality constant, since the augmented eigenfunctions are coupled via the boundary terms. 

%CV: the collision of $\tau$'s is ugly.
...
For two populations with sample sizes $M_1$ and $M_2$, the respective likelihoods $\Pr(y_1\given M_1)$ and $\Pr(y_2\given M_2)$ are similarly expanded into the modified Jacobi polynomials with coefficients $c_i^{(1)}$ and $c_i^{(2)}$, where the superscripts indicate the population. At time $\tau$ back in the paste, the marginal likelihood is
...
In this article, we use backward processes in discrete or continuous time to conveniently calculate probabilities of population allelic proportions of a bi-allelic locus backward in time conditional on site frequency spectra (SFS) from the present. This may be used to infer trajectories of population allelic proportions backward in time conditional on a sample from the present or to calculate joint site frequency spectra (SFS) of splitting populations. In discrete time, the algorithm is a variant of the forward-backward algorithm and thus makes use of the dynamic programming algorithm. In continuous time, orthogonal polynomials allow for even more convenient calculation. Furthermore, we introduce bi-allelic population genetic models with mutations only from the boundaries that provide us with time-reversible and non-reversible transition matrices or kernels. The non-reversible models are variants of the infinite site or Poisson-random-fields models \citep{Kimu69,Sawy92} and have stationary distributions.

The discrete models involve repeated multiplications with a transition matrix of dimension $(N+1)\times(N+1)$, where $N$ is the haploid population size. Generally, $N$ should be taken large to model the large (effective) population sizes usually encountered. For numerical reasons, however, $N$ should be small, because iteration of large matrices is time-consuming and numerical errors may accumulate. In any case, $N$ must be at least as big as the sample size $M$ to not lose information. A prior distribution must be assumed at time $t=S$ in the past. If this distribution is taken as the stationary distribution of the transition matrix, calculations simplify. At present, $t=0$, a sample is given and a likelihood is assumed, \ie\ the probability distribution of a sample of size $M$ with $y$ alleles given the population allelic proportion $\x{0}$. The method can be adjusted easily to an experimental setting with samples from multiple time points, as \eg\ is encountered in evolve-and-resequence experiments \citep{Kofl14}.

\citet{Zhao14} presented a method that also uses iteration of a transition matrix (in their case a Wright-Fisher model) with conditioning on the beginning and end of the chain. They transform the forward transition matrix by adding a ``fictitious force'' that assures that the final (in our case present) distribution is reached. While their method obviously produces the correct transition probabilities to produce the conditional trajectories of the population allelic proportion, it is more complicated than ours. It is not clear how this methods can be extended to samples from multiple time-points. Furthermore, the authors seem to envision a situation where the starting allelic proportion is known. Usually, however, no information or only a sample from the initial distribution is given.

With the continuous diffusion models, orthogonal polynomials are used. Conveniently, the dimension of the polynomials need not be higher than the sample size $M$, while the population size is assumed to converge to infinity. Thus, this approach is numerically preferable while still assuming the large effective population sizes usually encountered within population genetics. As with the discrete model also in the continuous diffusion case, \citet{Zhao13} presented a method that uses a similar setting but is more complicated and can be extended less easily. 
